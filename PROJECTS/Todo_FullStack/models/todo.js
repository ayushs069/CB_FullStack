// Models will work with the database.js to do CRUD operations
// Only this file will be responsible for db operations

const { ObjectId } = require("mongodb");
const {getDB} = require("../database/database");
const { v4: uuidv4 } = require('uuid');

// todoAppn is the name of the database
// todo is the name of the collection in it
...
async function getPriority() {
    ...
    if (!cnt) {
        ...
        // Fix: Access the updated document correctly
    ...
    static increasePriority(id){
        return new Promise(async (resolve, reject) => {
            try {
                ...
                //2. find tasks with less priority than current item & sort in desc order
                ...
                //3. fetch item with which we want to swap the priority
                ...
                //4. swap the priority of current item and previous item
    ...
    static decreasePriority(id){
        return new Promise(async (resolve, reject) => {
            try {
                ...
                //2. find tasks with high priority than current item & sort in asc order
                let allTasks = await todo.find({
                    priority:{
